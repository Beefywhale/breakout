...
Engine::Engine(sf::RenderWindow& window, Map map) {
	...
	textRenderer.setCharacterSize(fontSize);
...
void Engine::drawActors() {
	...
	for (auto actor : actors) {
		actor->update();
		textRenderer.setFillColor(sf::Color(actor->getColor().red, actor->getColor().green, actor->getColor().blue));
		textRenderer.setString(actor->getChar());
		textRenderer.setPosition(actor->getPosition().x * (fontSize - 2),
								actor->getPosition().y * (fontSize - 2)
		);
		actorPos.push_back(std::make_pair(actor->getPosition().x, actor->getPosition().y));
		m_window->draw(textRenderer);
	}
}
...
void Engine::drawTiles() {
    ...
    for (auto tilePair : m_map.getTiles()) {
		...
		textRenderer.setFillColor(sf::Color(i.getColor().red, i.getColor().green, i.getColor().blue));

		textRenderer.setString(i.getChar());
		textRenderer.setPosition(i.getPosition().x * (fontSize - 2),
